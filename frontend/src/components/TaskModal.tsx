...
export default function TaskModal({ open, onClose, onSubmit, task, isSubmitting }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md card p-6 shadow-xl animate-in">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display font-semibold text-ink">{task ? 'Edit task' : 'New task'}</h2>
          <button onClick={onClose} className="text-ink-tertiary hover:text-ink text-sm px-2 py-1 rounded hover:bg-layer-3 transition-colors">
            Close
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-ink mb-1.5">
              Due date <span className="text-ink-tertiary font-normal">(optional)</span>
            </label>
            <input
              type="date"
              className="input-base"
